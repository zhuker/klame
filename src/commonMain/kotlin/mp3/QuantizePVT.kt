...
class QuantizePVT {
    ...
    private fun compute_ath(gfp: LameGlobalFlags) {
        ...
        val ath = internal_flags.ATH!!
        val ATH_l = ath.l
        val ATH_psfb21 = ath.psfb21
        val ATH_s = ath.s
        val ATH_psfb12 = ath.psfb12
    ...
    private fun set_pinfo(gfp: LameGlobalFlags,
        ...
        while (sfb < sfb2) {
            ...
            en1 = 1e15f
            pinfo.en[gr][ch][sfb] = (en1 * en0).toDouble()
            pinfo.xfsf[gr][ch][sfb] = (en1 * l3_xmin[sfb] * xfsf[sfb] / bw).toDouble()
            ...
            pinfo.thr[gr][ch][sfb] = (en1 * Math.max(en0 * ratio.thm.l[sfb], ath.l[sfb])).toDouble()

            /* there is no scalefactor bands >= SBPSY_l */
            pinfo.LAMEsfb[gr][ch][sfb] = 0.0
            if (cod_info.preflag != 0 && sfb >= 11)
                pinfo.LAMEsfb[gr][ch][sfb] = (-ifqstep * pretab[sfb]).toDouble()

            if (sfb < Encoder.SBPSY_l) {
                /* scfsi should be decoded by caller side */
                assert(scalefac[sfb] >= 0)
                pinfo.LAMEsfb[gr][ch][sfb] -= (ifqstep * scalefac[sfb]).toDouble()
            }
            sfb++
        } /* for sfb */
        ...
        if (cod_info.block_type == Encoder.SHORT_TYPE) {
            ...
            while (sfb < Encoder.SBMAX_s) {
                ...
                for (i in 0..2) {
                    ...
                    pinfo.en_s[gr][ch][3 * sfb + i] = (en1 * en0).toDouble()
                    pinfo.xfsf_s[gr][ch][3 * sfb + i] = ((en1 * l3_xmin[sfb2]
                    ...
                    pinfo.thr_s[gr][ch][3 * sfb + i] = (en1 * Math.max(en0 * ratio.thm.s[sfb][i],
                            ath!!.s[sfb])).toDouble()

                    /* there is no scalefactor bands >= SBPSY_s */
                    pinfo.LAMEsfb_s[gr][ch][3 * sfb + i] = -2.0 * cod_info.subblock_gain[i]
                    if (sfb < Encoder.SBPSY_s) {
                        pinfo.LAMEsfb_s[gr][ch][3 * sfb + i] -= (ifqstep * scalefac[sfb2]).toDouble()
                    }
                    sfb2++
                }
                sfb++
