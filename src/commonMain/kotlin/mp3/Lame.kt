    ...
    private fun fill_buffer_resample(gfp: LameGlobalFlags,
        ...
        if (gfc.fill_buffer_resample_init == 0) {
            ...
            while (j <= 2 * bpc) {
                ...
                i = 0
                val blackfilt_j = gfc.blackfilt[j]!!
                while (i <= filter_l) {
                    blackfilt_j[i] = blackman(i - offset, fcn,
                            filter_l)
                    sum += blackfilt_j[i]
                    i++
                }
        ...
        while (k < desired_len) {
            ...
            var xvalue = 0f
            i = 0
            val blackfilt_joff = gfc.blackfilt[joff]!!
            while (i <= filter_l) {
...
//                run {
                    ...
                    PSY.mask_adjust = gfp.maskingadjust
                    PSY.mask_adjust_short = gfp.maskingadjust_short
...
//                }
        ...
        if (gfp.tune) {
            PSY.mask_adjust += gfp.tune_value_a
            PSY.mask_adjust_short += gfp.tune_value_a
        }
        ...
        if (gfp.athaa_type < 0)
            ...
            ATH.useAdjust = gfp.athaa_type

        /* initialize internal adaptive ATH settings -jd */
        ATH.aaSensitivityP = Math.pow(10.0, gfp.athaa_sensitivity / -10.0).toFloat()
        ...
        if (internal_flags.nsPsy.attackthre < 0)
            internal_flags.nsPsy.attackthre = PsyModel.NSATTACKTHRE
        if (internal_flags.nsPsy.attackthre_s < 0)
            internal_flags.nsPsy.attackthre_s = PsyModel.NSATTACKTHRE_S.toFloat()
    ...
    fun lame_print_internals(gfp: LameGlobalFlags) {
        ...
        pc = when (gfp.mode) {
            MPEGMode.JOINT_STEREO -> "joint stereo"
            MPEGMode.STEREO -> "stereo"
            MPEGMode.DUAL_CHANNEL -> "dual channel"
            MPEGMode.MONO -> "mono"
            MPEGMode.NOT_SET -> "not set (error)"
            else -> "unknown (error)"
        }
        ...
        pc = if (gfp.useTemporal == true) "yes" else "no"
     ...
     */
    fun lame_encode_buffer_sample(gfp: LameGlobalFlags,
...
//                    }
    ...
    fun lame_encode_buffer(gfp: LameGlobalFlags,
