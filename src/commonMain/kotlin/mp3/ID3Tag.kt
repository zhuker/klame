...
class ID3Tag {
    ...
    private fun copyV1ToV2(gfp: LameGlobalFlags, frame_id: Int,
        ...
        val tag_spec = gfc.tag_spec!!
        val flags = tag_spec.flags
        id3v2_add_latin1(gfp, frame_id, null, null, s)
        tag_spec.flags = flags
    }
    ...
    fun id3tag_init(gfp: LameGlobalFlags) {
        val gfc = gfp.internal_flags!!
        val tag_spec = ID3TagSpec()
        gfc.tag_spec = tag_spec
        tag_spec.genre_id3v1 = GENRE_NUM_UNKNOWN
        tag_spec.padding_size = 128
    ...
    fun id3tag_add_v2(gfp: LameGlobalFlags) {
        val gfc = gfp.internal_flags!!
        val tag_spec = gfc.tag_spec!!
        tag_spec.flags = tag_spec.flags and V1_ONLY_FLAG.inv()
        tag_spec.flags = tag_spec.flags or ADD_V2_FLAG
    }

    fun id3tag_v1_only(gfp: LameGlobalFlags) {
        val gfc = gfp.internal_flags!!
        val tag_spec = gfc.tag_spec!!
        tag_spec.flags = tag_spec.flags and (ADD_V2_FLAG or V2_ONLY_FLAG).inv()
        tag_spec.flags = tag_spec.flags or V1_ONLY_FLAG
    }
    ...
    fun id3tag_v2_only(gfp: LameGlobalFlags) {
        ...
        val tag_spec = gfc.tag_spec!!
        tag_spec.flags = tag_spec.flags and V1_ONLY_FLAG.inv()
        tag_spec.flags = tag_spec.flags or V2_ONLY_FLAG
    }

    fun id3tag_space_v1(gfp: LameGlobalFlags) {
        val gfc = gfp.internal_flags!!
        val tag_spec = gfc.tag_spec!!
        tag_spec.flags = tag_spec.flags and V2_ONLY_FLAG.inv()
        tag_spec.flags = tag_spec.flags or SPACE_V1_FLAG
    }
    ...
    fun id3tag_set_pad(gfp: LameGlobalFlags, n: Int) {
        ...
        val tag_spec = gfc.tag_spec!!
        tag_spec.flags = tag_spec.flags and V1_ONLY_FLAG.inv()
        tag_spec.flags = tag_spec.flags or PAD_V2_FLAG
        tag_spec.flags = tag_spec.flags or ADD_V2_FLAG
        tag_spec.padding_size = n
    }
    ...
    fun id3tag_set_albumart(gfp: LameGlobalFlags,
        ...
        if (2 < size && image[0] == 0xFF.toByte() && image[1] == 0xD8.toByte()) {
            mimetype = MimeType.MIMETYPE_JPEG
                ...
                && makeString(image, 1, 3, ASCII).startsWith("PNG")) {
            mimetype = MimeType.MIMETYPE_PNG
        } else if (4 < size && makeString(image, 1, 3, ASCII).startsWith("GIF8")) {
            mimetype = MimeType.MIMETYPE_GIF
        } else {
            return false
        }
        val tag_spec = gfc.tag_spec!!
        if (tag_spec.albumart != null) {
            tag_spec.albumart = null
            tag_spec.albumart_size = 0
            tag_spec.albumart_mimetype = MimeType.MIMETYPE_NONE
        }
        if (size < 1) {
            return true
        }
        tag_spec.albumart = ByteArray(size)
        if (tag_spec.albumart != null) {
            System.arraycopy(image, 0, tag_spec.albumart!!, 0, size)
            tag_spec.albumart_size = size
            tag_spec.albumart_mimetype = mimetype
            tag_spec.flags = tag_spec.flags or CHANGED_FLAG
            id3tag_add_v2(gfp)
        }
        return true
    }
    ...
    private fun id3v2_add_ucs2(gfp: LameGlobalFlags, frame_id: Int,
        ...
        if (gfc != null) {
            val tag_spec = gfc.tag_spec!!
            var node = findNode(tag_spec, frame_id, null)
            if (isMultiFrame(frame_id)) {
                while (node != null) {
                    if (isSameLang(node.lng.orEmpty(), lang)) {
                        if (isSameDescriptorUcs2(node, desc)) {
                            break
                        }
                    }
                    node = findNode(tag_spec, frame_id, node)
                }
            }
            if (node == null) {
                node = FrameDataNode()
                appendNode(tag_spec, node)
            }
            node.fid = frame_id
            ...
            node.txt.enc = 1
            tag_spec.flags = tag_spec.flags or (CHANGED_FLAG or ADD_V2_FLAG)
        }
    }
    ...
    private fun id3v2_add_latin1(gfp: LameGlobalFlags,
        ...
        if (gfc != null) {
            ...
            val tag_spec = gfc.tag_spec!!
            node = findNode(tag_spec, frame_id, null)
            if (isMultiFrame(frame_id)) {
                while (node != null) {
                    if (isSameLang(node.lng.orEmpty(), lang)) {
    ...
    private fun set_frame_comment(frame: ByteArray, framePos: Int,
        ...
        if (n > 10) {
            ...
            /* 3 bytes language */
            val lng = node.lng.orEmpty()
            frame[framePos++] = lng[0].toByte()
            frame[framePos++] = lng[1].toByte()
            frame[framePos++] = lng[2].toByte()
            /* descriptor with zero byte(s) separator */
            if (node.dsc.enc != 1) {
                framePos = writeChars(frame, framePos, node.dsc.l.orEmpty(), 0,
                        node.dsc.dim)
                frame[framePos++] = 0
            } else {
                framePos = writeUcs2s(frame, framePos, node.dsc.l.orEmpty(), 0,
                        node.dsc.dim)
                frame[framePos++] = 0
                frame[framePos++] = 0
            }
            /* comment full text */
            if (node.txt.enc != 1) {
                framePos = writeChars(frame, framePos, node.txt.l.orEmpty(), 0,
                        node.txt.dim)
            } else {
                framePos = writeUcs2s(frame, framePos, node.txt.l.orEmpty(), 0,
                        node.txt.dim)
            }
        }
        return framePos
        ...
        if (fieldvalue != null && fieldvalue.length != 0) {
            ...
            if (frame_id != 0) {
                if (id3tag_set_textinfo_latin1(gfp, fieldvalue,
                    ...
                    tag_spec.values.add(fieldvalue)
                    tag_spec.num_values++
                }
            }
            tag_spec.flags = tag_spec.flags or CHANGED_FLAG
    ...
    fun lame_get_id3v2_tag(gfp: LameGlobalFlags?,
        ...
        val tag_spec = gfc.tag_spec!!
        if (tag_spec.flags and V1_ONLY_FLAG != 0) {
            return 0
        }
        run {
            /* calculate length of four fields which may not fit in verion 1 tag */
            val title_length = tag_spec.title.orEmpty().length
            val artist_length = tag_spec.artist.orEmpty().length
            val album_length = tag_spec.album.orEmpty().length
            val comment_length = tag_spec.comment.orEmpty().length
            /* write tag if explicitly requested or if fields overflow */
            if (tag_spec.flags and (ADD_V2_FLAG or V2_ONLY_FLAG) != 0
                    || title_length > 30 || artist_length > 30
                    || album_length > 30 || comment_length > 30
                    || tag_spec.track_id3v1 != 0 && comment_length > 28) {
                var tag_size: Int
                var p: Int
                ...
                tag_size = 10
                i = 0
                while (i < tag_spec.num_values) {
                    tag_size += 6 + tag_spec.values[i].length
                    ++i
                }
                if (tag_spec.albumart != null && tag_spec.albumart_size != 0) {
                    when (tag_spec.albumart_mimetype) {
                ...
                if (tag_spec.flags and PAD_V2_FLAG != 0) {
                    /* add some bytes of padding */
                    tag_size += tag_spec.padding_size
                }
                if (size < tag_size) {
                    return tag_size
                }
                ...
                i = 0
                while (i < tag_spec.num_values) {
                    p = set_frame_custom(buffer, p, tag_spec.values[i]
                            .toCharArray())
                    ++i
                }
                if (albumart_mime != null) {
                    p = set_frame_apic(buffer, p, albumart_mime.toCharArray(),
                            tag_spec.albumart, tag_spec.albumart_size)
                }
    ...
    fun id3tag_write_v2(gfp: LameGlobalFlags): Int {
        val gfc = gfp.internal_flags!!
        val tag_spec = gfc.tag_spec!!
        if (tag_spec.flags and CHANGED_FLAG != 0 && 0 == tag_spec.flags and V1_ONLY_FLAG) {
    ...
    fun lame_get_id3v1_tag(gfp: LameGlobalFlags?,
        ...
        val tag_spec = gfc.tag_spec!!
        if (tag_spec.flags and CHANGED_FLAG != 0 && 0 == tag_spec.flags and V2_ONLY_FLAG) {
            var p = 0
            val pad = (if (tag_spec.flags and SPACE_V1_FLAG != 0) ' ' else 0.toChar()).toInt()
            val year: String
            ...
            p = set_text_field(buffer, p, tag_spec.title, 30, pad)
            p = set_text_field(buffer, p, tag_spec.artist, 30, pad)
            p = set_text_field(buffer, p, tag_spec.album, 30, pad)
            year = String.format("%d", Integer.valueOf(tag_spec.year))
            p = set_text_field(buffer, p, if (tag_spec.year != 0) year else null,
                    4, pad)
            /* limit comment field to 28 bytes if a track is specified */
            p = set_text_field(buffer, p, tag_spec.comment,
                    if (tag_spec.track_id3v1 != 0) 28 else 30, pad)
            if (tag_spec.track_id3v1 != 0) {
                /* clear the next byte to indicate a version 1.1 tag */
                buffer[p++] = 0
                buffer[p++] = tag_spec.track_id3v1.toByte()
            }
            buffer[p++] = tag_spec.genre_id3v1.toByte()
            return tag_size
        }
        return 0
    }
